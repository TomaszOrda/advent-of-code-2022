...
class VolcanoComplex:
    ...
    def max_flow_rate(self):
        return self.max_flow_rate_h("AA", set())

    def max_flow_rate_h(self, start_valve, visited_valves: set[str], time_left=26):
        cache_key = (
            "h",
            start_valve,
        ...
        for next_valve in self.flow_valves:
            if next_valve not in visited_valves:
                ...
                if new_time_left > 0:
                    ...
                    remaining_flow = self.max_flow_rate_h(
                        next_valve,
        ...
        possible_flows.append(self.max_flow_rate_e("AA", visited_valves))
        self.cache[cache_key] = max(possible_flows)
        return self.cache[cache_key]

    def max_flow_rate_e(self, start_valve, visited_valves: set[str], time_left=26):
        cache_key = (
            "e",
            start_valve,
            ''.join(sorted(list(visited_valves))),
        ...
        for next_valve in self.flow_valves:
            if next_valve not in visited_valves:
                ...
                if new_time_left > 0:
                    additional_flow = new_time_left * self.valves[next_valve].flow_rate
                    remaining_flow = self.max_flow_rate_e(
                        next_valve,
                        visited_valves.union([next_valve]),
                        new_time_left)
                    possible_flows.append(additional_flow + remaining_flow)
